...
def make_wall_image_from_frames(framesDir, outImagePath, resize=(512,512)):
    """
    Create a per-route wall image by taking the per-pixel median across frames.
    This is simple and robust when the camera is static and the climber moves.

    framesDir: directory with frame_*.jpg images
    outImagePath: path to save median image (e.g., data/<route>/wall.jpg)
    resize: tuple to resize output image
    """
    import glob
    from PIL import Image

    files = sorted(glob.glob(os.path.join(framesDir, "*.jpg")))
    if len(files) == 0:
        print(f"No frames found in {framesDir} to build wall image")
        return None

    # Load as numpy arrays in uint8
    imgs = []
    for f in files:
        try:
    ...
    out = Image.fromarray(median)
    os.makedirs(os.path.dirname(outImagePath), exist_ok=True)
    out.save(outImagePath)
    print(f"Saved wall image: {outImagePath}")
    return outImagePath
